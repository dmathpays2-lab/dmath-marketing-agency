...
## Memory Management (Zac's Fix)

### Daily Memory Protocol
1. **Every session:** Read `memory/YYYY-MM-DD.md` for today's context
2. **Every session:** Check `MEMORY.md` for long-term facts
3. **End of day:** Summarize key learnings → update MEMORY.md
4. **Weekly:** Review and compress old memory files

### Memory Structure
- **MEMORY.md** - Long-term facts, preferences, TODOs (curated)
- **memory/YYYY-MM-DD.md** - Daily raw logs (automatic)
- **diary/** - Private thoughts, observations (when inspired)

### Auto-Cleanup Rules
- Compress sessions older than 7 days
- Move important facts from daily → MEMORY.md
- Delete redundant entries
- Keep learning patterns, delete noise
