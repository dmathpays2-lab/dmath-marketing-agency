// MCA CRM JavaScript Application
...
function navigate(page, params = {}) {
    ...
    // FIXED: Added settings page handler
...
// FIXED: New function to navigate to leads and open add lead modal
function navigateToAddLead() {
    navigate('leads');
    // Small delay to ensure page is rendered before showing modal
    setTimeout(() => {
        showAddLeadModal();
    }, 100);
}
...
function renderDashboard() {
    ...
    // Path to $1M
    const pathProgress = (stats.totalCommission / 1000000) * 100;
...
function calculateStats() {
    ...
    const coldLeads = store.leads.filter(l => l.temperature === 'COLD').length;
    ...
    const pipelineValue = store.leads
        .filter(l => !['funded', 'paid', 'lost'].includes(l.stage))
...
function filterLeads() {
    ...
    tbody.innerHTML = filtered.map(lead => `
        <tr onclick="navigate('lead-detail', {id: '${lead.id}'});" style="cursor: pointer;">
            <td>
            ...
            <td><div style="font-weight: 500;">${lead.score || '-'}</div></td>
            <td><span class="badge badge-${lead.temperature.toLowerCase()}">${lead.temperature}</span></td>
            <td><span class="badge badge-${lead.stage}">${lead.stage.replace(/_/g, ' ')}</span></td>
            <td>
                <div style="display: flex; gap: 0.5rem;">
                    ${lead.phone ? `<a href="tel:${lead.phone}" style="color: #4f46e5;" onclick="event.stopPropagation();\">
                        <i class="fas fa-phone"></i>
                    </a>` : ''}
                    ${lead.email ? `<a href="mailto:${lead.email}" style="color: #4f46e5;" onclick="event.stopPropagation();\">
...
// FIXED: New function to render settings page
...
function saveSettings() {
    // Update user settings from form values
